...
; (function (factory) {
    ...
    EvoCalendar = (function () {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.initials = {
                ...
                dates: {
                    ...
                    es: {
                        ...
                        noEventForToday: "No hay ninguna reserva",
                        noEventForThisDay: "No hay ninguna reserva",
            ...
            _.$label = {
                ...
                defaultmonths: _.initials.dates["en"].months,
    ...
    EvoCalendar.prototype.buildTheBones = function () {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            for (var i = 0; i < _.$label.months.length; i++) {
                markup += '<li class="month" role="button" data-month-val="' + i + '">' + _.$label.months[i] + '</li>';
            }
            markup += '</ul>';
            markup += '</div>' + 
                '<button class="return-button" role="button" id="return-button">' +
                '<div class="event-info"><p class="event-title">👈 Volver a la página de selección</p></div>' +
                '</button>' + 
            '</div>';

            // events
            markup += '<div class="calendar-events">' +
                '<div class="event-header-and-addbuttom">' + 
                    '<div class="event-header"><p></p></div>' +

                    '<button class="addevent-button" role="button" id="addevent">' +
                    '<div class="addevent-button-icon"><p class="addevent-button-icon-plus">+</p></div>' +
                    '<div class="event-info"><p class="event-title">Nueva reserva</p></div>' +
                    '</button>' +
                '</div>' + 
            ...
            btn.onclick = function () {
                ...
                const ul1 = document.getElementById('s.hora.inicio');
                ul1.childNodes.forEach(li => {
                    li.disabled = false;
                });
                const ul2 = document.getElementById('s.hora.final');
                ...
                fetch("../reservas.json")
                    ...
                    .then(jsondata => {
                        for (let i = 0; i <= jsondata.length; i++) {
                            if (jsondata[i] != null && jsondata[i]["Date"] != null && jsondata[i]["Elecion"] != null) {
                                if (jsondata[i]["Date"] === _.$active.date) {
                                    if (jsondata[i]["Elecion"] === eCookie) {
                                        let hi = jsondata[i]["Hora_inicio"]
                                        let hf = jsondata[i]["Hora_final"]
                                        ...
                                        for (let i = hii; i <= hfi; i += 5) {
                                            var s = i.toString();
                                            var minutes = s.substring(s.length - 2, s.length);
                                            var hours = s.substring(0, s.length - 2);
                                            hours = pad(hours, 2);

                                            var elements = getElementsById(hours + ":" + minutes);
    ...
    EvoCalendar.prototype.addEventList = function (event_data) {
        ...
        markup += '</div>';
        if (event_data.showTrash) {
            markup += '<button class="event-trash" onclick="trash_button_onClick(this);"><img src="images/trash_white.png" id="button-trash"></button>';
        }
        markup += '</div>';
        eventListEl.append(markup);
    ...
    EvoCalendar.prototype.buildCalendar = function () {
        ...
        _.calculateDays();
        title = _.$label.months[_.$active.month]
        ...
        for (var i = 0; i < 9; i++) { // this loop is for is weeks (rows)
            for (var j = 0; j < _.$label.days.length; j++) { // this loop is for weekdays (cells)
                if (day <= _.monthLength && (i > 0 || j >= _.startingDay)) {
                    ...
                    var thisDay = _.formatDate(_.$label.defaultmonths[_.$active.month] + ' ' + day + ' ' + _.$active.year, _.options.format);
    ...
    EvoCalendar.prototype.addEventIndicator = function (event) {
        ...
        function appendDot(date) {
            ...
            if (thisDate.find('span.event-indicator > .type-bullet > .type').length === 0) {
                htmlToAppend = '<div class="type-bullet"><div ';

                htmlToAppend += 'class="type"'
    ...
    ****************/
    ...
    EvoCalendar.prototype.selectYear = function (event) {
        ...
        var newStyleLeft = ""
        var newStyleRight = ""
        if (_.$active.year < _.$current.year) {
            ++_.$active.year;
            newStyleLeft = "none"
        } else if (_.$active.year == _.$current.year) {
            newStyleLeft = "none"
        } else if (_.$active.year > 2100) {
            _.$active.year = 2100;
            newStyleRight = "none"
        } else if (_.$active.year == 2100) {
            newStyleRight = "none"
        }
        const collection1 = document.getElementsByClassName("chevron-arrow-left")
        for (let i = 0; i < collection1.length; i++) {
            collection1[i].style.display = newStyleLeft;
            collection1[i].parentElement.style.cursor = newStyleLeft == "none" ? "default" : "";
        }
        const collection2 = document.getElementsByClassName("chevron-arrow-right")
        for (let i = 0; i < collection2.length; i++) {
            collection2[i].style.display = newStyleRight;
            collection2[i].parentElement.style.cursor = newStyleRight == "none" ? "default" : "";
        }
    ...
    EvoCalendar.prototype.selectMonth = function (event) {
        ...
        $(_.$elements.calendarEl).trigger("selectMonth", [_.$label.months[_.$active.month], _.$active.month])
    };
